...
The goal of this section is to find a transform that moves the space $\mathbb{R}^p$ \newnot{symbol:real} to the simplex $\mathbb{S}^p$. \newnot{symbol:simp} The simplex $\mathbb{S}^p$ is a space defined by the constraints $\{x_i: 0<x_i<1, \sum_{j=1}^px_j <1 \}$, and the extra term $x_{p+1}=1-\sum_{j=1}^p x_j$ ensures the total sums to 1.  
