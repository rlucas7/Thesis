 \chapter{Dirichlet Variable Selection For Decision Trees: The DiVaS method}
\label{ch:var_select}
...
\section{A Simulated Example}\label{sec:ase}
...
The probability of being the majority class in any of the four regions of the Figure \ref{fig:awesome_image2} is 90\%. Therefore no perfect classifier exists, yet very good classifiers are possible. We next generate additional covariates independently according to the mixture strategy described in the previous paragraph, with the continuous uniform supported on the interval $(0,20)$. We look at the cases of $d=100 \text{ and } 400$. To summarize the responses are simulated according to scaled Bernoulli random variables in each of the regions shown in Figure  \ref{fig:awesome_image2}. Addition covariates are simulated, 98 and 398 for $d=100$ and $400$ respectively. The 98 or 398 additional covariates are simulated from a $50\%$ discrete mixture of two normals where both normals have unit variances and the two different normals have different means simulated independently from a continuous uniform distribution on the region (0,20). This simulation of a discrete mixture of normals is done independently for each of the additional, or noise, covariates.

The trees generated by the three tree methods are shown in Figures \ref{fig:awesome_image1}-\ref{fig:awesome_image3}. The best tree found by our method is the generative tree of the data, and therefore this classifier achieves the Bayes error rate for this example. We simulate several Markov chains are simulated to guard against trapping in local optima. This is not a new strategy for MCMC with many local optima and was proposed by CGM \cite{chipman1998bayesian} for decision tree MCMC samplers to aid exploration of the decision tree space and to prevent getting stuck in one of the many local optima.  We tried a few chain lengths (500, 1000, and 2000) of each chain and fitted 10 chains on each data set. The results were the same, regardless of the chain length.
...
Greedy &   11.6 \%    \\
 CGM &     $62.4 \pm 21.11$ \% \\
   Weighted & $10.8 \pm 2.46$\%   \\
      Random Forest & $13.6 \pm 1.01$\%   \\
      \hline
  \end{tabular}
...
Greedy &  24.8 \%     \\
 CGM &    $57.2 \pm 16.21$\% \\
   Weighted &  $10.0 \pm 1.11$\%  \\
    Random Forest   & $22.4 \pm 1.22$\%   \\
      \hline
  \end{tabular}
    \caption[Misclassification Probabilities of the three tree fitting methods for $d=400$.]{Misclassification Probabilities of the three tree fitting methods for $d=400$. The entries after the $\pm$ indicate estimated standard errors.}
...
\section{The Internet Advertisements Data}
...
Greedy & 5.578\% \\ %0.0557598      \\
 CGM &   $7.2 \pm 3.11$\% \\%.0716912   \\
   Weighted & $5.598 \pm 1.27$\% \\%0.0559853   \\
Random Forest & $6.2 \pm 2.16$\% \\
      \hline
  \end{tabular}
